            ...
            if(driver) {
                ...
                if (confirmed) {
                    ...
                    api.updateLockedData(driver.id, null, payDate, 'unlock').then(() => {
                        ...
                        // --- FIX: --- 
                        // The "high-performance revert" was flawed. It failed to recalculate percentiles
                        // (which require the whole population) and did not re-apply manual overrides.
                        // Replacing it with the full `processDataForSelectedDate` function fixes both issues,
                        // as this function correctly re-calculates *all* data, including percentiles,
                        // and then re-applies any `savedOverrides`.
                        
                        // 1. Re-run the full, correct calculation for all drivers on this date.
                        processDataForSelectedDate();
                        
                        // 2. Re-render the table with the corrected data.
                        filterAndRenderTable();
                        // --- END FIX ---
    ...
    tableHead.addEventListener('click', e => {
        ...
        if (actionLink) {
            ...
            if (action.startsWith('sort')) {
                ...
                // 1. Sort the 'processedDriversForDate' array, not 'driversForDate'
                processedDriversForDate.sort((a, b) => {
                    // 'totalTpog' is already calculated on these objects, no need to re-run calc
                    let valA = a[key];
                    let valB = b[key];
                ...
                // 2. Call filterAndRenderTable() to re-apply filters on the sorted list
                filterAndRenderTable();
            }
            // --- END OF MODIFICATION ---
    ...
    async function attemptLogin() {
        ...
        if (result.status === 'success') {
            ...
            if (userRole !== 'Admin') {
                ...
                // MODIFIED: Only hide manual report button if NOT Marketing
            ...
            if (userRole === 'Marketing') {
                ...
                // Show the new help button
                const helpBtn = document.getElementById('marketing-help-btn');
                if (helpBtn) helpBtn.classList.remove('hidden');
            }
            // --- END NEW LOGIC ---

            // --- NEW LOGIC FOR DISPATCHER/DRIVER REP ---
            if (userRole === 'Dispatcher' || userRole === 'Driver Rep') {
                // Hide Driver Archive
                const navArchiveLink = document.getElementById('nav-archive');
                if (navArchiveLink) navArchiveLink.style.display = 'none';
...
function initDispatcherView(
    ...
    async function saveOverrides(overridesToSave, noteData) { // <-- MODIFIED SIGNATURE
        ...
        try {
            ...
            const overridePromise = fetch(config.DISPATCHER_OVERRIDES_URL, {
                method: 'POST',
