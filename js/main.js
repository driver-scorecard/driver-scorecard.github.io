 ...
 */
import { columnConfig, DRIVER_DATA_URL, MILEAGE_DATA_URL, ALL_SAFETY_DATA_URL, DAYS_TAKEN_HISTORY_URL, DISPATCHER_OVERRIDES_URL } from './config.js';
...
userProfilesTableBody.addEventListener('click', async (e) => {
    ...
    document.getElementById('edit-panel').addEventListener('click', async (e) => {
        ...
        if (e.target.closest('#return-to-default-btn')) {
            ...
            if (confirm("This will delete all saved overrides for this driver for this week. Continue?")) {
                ui.showLoadingOverlay();
                const payDate = driverForDate.pay_date.split('T')[0];
    ...
    tableBody.addEventListener('click', e => {
        ...
        if (e.target.closest('td[data-key="milesWeek"]') || e.target.closest('td[data-key="samsaraDistance"]')) {
            ...
            const driver = driversForDate.find(d => d.id == e.target.closest('tr').dataset.driverId);
            if (!driver) return;
...
function switchView(viewName) {
    ...
    if (viewName === 'table') {
        ...
        if (!document.body.classList.contains('dispatcher-initialized')) {
            initDispatcherView(currentUser);
...
function initDispatcherView(currentUser) {
    // --- STATE MANAGEMENT ---
    let allDriverData = {};
    let allPayDates = [];
    let savedOverrides = {};
    ...
    async function init(forceTutorial = false) {
        ...
        if (isTutorialMode) {
            ...
            const [savedOverridesData, driverList, mileageData, samsaraData, changelogData] = await Promise.all([
                fetchData(DISPATCHER_OVERRIDES_URL, 'Dispatcher Overrides'),
                fetchData(DRIVER_DATA_URL, 'Driver List'),
                fetchData(MILEAGE_DATA_URL, 'Mileage Data'),
                fetchData(ALL_SAFETY_DATA_URL, 'Samsara Data'),
                fetchData(DAYS_TAKEN_HISTORY_URL, 'Activity Logs')
            ]);
