...
export function processDriverDataForDate(driversForDate, mileageData, settings, allSafetyData, overriddenDistances, daysTakenHistory, dispatcherOverrides, allDrivers) {
    ...
    if (mileageData.length > 0 || settings.weeksOutMethod === 'daysOff' || settings.weeksOutMethod === 'dailyAccrual' || settings.weeksOutMethod === 'fullWeeksOnly') {
                ...
                if (settings.weeksOutMethod === 'dailyAccrual') {
                    ...
                    for (const record of allRecordsForDriver) {
                        ...
                        if (recordPayDateStr === selectedDateStr) {
                            ...
                            // **HERE IS THE FIX**: If reset is triggered this week, set balance to 0
                        ...
                        // Apply reset for the *next* iteration
                        if (resetTriggeredThisWeek) {
                            earnedDays = 0;
                            takenDays = 0;
                            // continuousDayStreak is already 0 from the inner loop
                        } else {
                    ...
                    for (const record of allRecordsForDriver) {
                        ...
                        if (recordPayDateStr === selectedDateStr) {
                            ...
                            // **HERE IS THE FIX**: If reset is on this week, show 0s for all counters
                        ...
                        // Check for reset for the *next* iteration
                        if (hasNotStartedInWeek) {
                            streak = 0;
                            earnedDays = 0;
                            takenDays = 0;
                        } else {
                            // Original logic if no reset
                            takenDays += daysOffInWeek;
        ...
        driversForDate.forEach(driver => {
            ...
            // --- NEW LINE ---
            // Save the distance source to the driver object so it gets locked.
