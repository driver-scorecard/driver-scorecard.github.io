...
export async function loadDispatcherOverrides() {
    ...
    const result = await fetchWithRetry(DISPATCHER_OVERRIDES_URL, "Dispatcher Overrides");
...
export async function loadWeeklyNotes() {
    ...
    const result = await fetchWithRetry(WEEKLY_NOTES_URL, "Weekly Notes");
...
export async function loadLockedData() {
    ...
    const result = await fetchWithRetry(LOCKED_DATA_URL, "Locked Data");
