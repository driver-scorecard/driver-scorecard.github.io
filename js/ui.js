    ...
    data.forEach((driver, index) => {
        ...
        currentOrderedKeys.forEach(key => {
            ...
            if (key === 'availableOffDays') {
                // If an override value exists directly on the driver object, use it. Otherwise, use the calculated value.
                ...
                // If an override value exists directly on the driver object, use it. Otherwise, use the calculated value.
                const escrowValue = driver.hasOwnProperty('escrowDeduct') ? driver.escrowDeduct : (reportData ? reportData.escrowDeduct : 0);
                if (escrowValue > 0) {
...
export function openEditPanel(driverId, state) {
    ...
    const fieldsToExclude = ['id', 'name', 'totalTpog', 'actions', 'contract_type', 'dispatcher', 'team', 'franchise', 'company', 'gross', 'rpm', 'estimatedNet', 'bonuses', 'penalties', 'speeding_over11mph', 'speeding_over16mph', 'samsaraDistance', 'availableOffDays', 'escrowDeduct', 'offDays'];
...
export function openActivityHistoryModal(driver, mileageData, settings, daysTakenHistory, dispatcherOverrides) {
    ...
    let historyHtml = '';
    const driverMileageMap = new Map(driverMileageRecords.map(m => [m.date, m.movement || 0]));
